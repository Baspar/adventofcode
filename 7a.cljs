...
(def i (map (fn [x] {:inside (map (fn [y] (apply str (butlast (rest y)))) (re-seq #"\[[^\]]*\]" x))
                     :outside (split x #"\[[^\]]*\]")})
            i))

(defn abba? [s] (let [part2 (rest s)
                      part1 (butlast s)

                      pairs-letters (apply mapv vector [part1 part2])

                      pairs-pairs (apply mapv vector [(butlast (butlast pairs-letters)) (rest (rest pairs-letters))])

                      return (some (fn [[x y]] (and (not= x y) (= x (reverse y)))) pairs-pairs)]
                  return))

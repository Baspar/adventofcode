...
(def inputs "wysextplwqpvipxdv[srzvtwbfzqtspxnethm]syqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt
...
(def i (map (fn [x] {:inside (map (fn [y] (apply str (butlast (rest y))))
                                  (re-seq #"\[[^\]]*\]" x))
                     :outside (split x #"\[[^\]]*\]")})
            i))

(defn get-all-aba-or-nil [string]
  (let [part2 (rest string)
        part1 (butlast string)

        pairs-letters (apply mapv vector [part1 part2])

        pairs-pairs (apply mapv vector [(butlast pairs-letters)
                                        (rest pairs-letters)])

        ab-ba-pairs (filter (fn [[x y]] (and (not= x y)
                                             (= x (reverse y))))
                            pairs-pairs)

        aba-pairs (map (fn [[x [_ y]]] (conj x y))
                       ab-ba-pairs)

        stringify (map #(apply str %)
                       aba-pairs)

        return (when (pos? (count stringify))
                 stringify)]
    return))


(def i (map (fn [x] (let [inside (keep get-all-aba-or-nil (:inside x))
                          all-inside (apply concat inside)
                          distinct-inside (distinct all-inside)
                          inside-patterns (map #(apply str (rest %)) distinct-inside)

                          outside (keep get-all-aba-or-nil (:outside x))
                          all-outside (apply concat outside)
                          distinct-outside (distinct all-outside)
                          outside-patterns (map #(apply str (butlast %)) distinct-outside)]
                      {:inside inside-patterns :outside outside-patterns}))
            i))


(def i (map (fn [x] (let [inside (:inside x)
                          outside (:outside x)

                          all-pattern (concat inside outside)

                          freq (frequencies all-pattern)

                          max-freq (last (sort-by second freq))]
                      max-freq))
            i))

(def i (filter (fn [x] (and (not (nil? x))
                            (< 1 (second x))))
               i))

(require '[clojure.string :as string :refer [split join replace]])
(require '[clojure.pprint :refer [pprint]])

;; (def input "aaaaa-bbb-z-y-x-123[abxyz]
;; a-b-c-d-e-f-g-h-987[abcde]
;; not-a-real-room-404[oarel]
;; totally-real-room-200[decoy]")
(def input "bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
(def real-score (map (fn [x]
                       (let [name (name-of x)
                             ...
                             real-checksum (join "" (take 5 (apply concat freq-order-without-tie)))

                             valid? (= checksum real-checksum)]
                         (if valid? id 0)))
                     lines))
